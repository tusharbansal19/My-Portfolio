...
Just ask me anything about Tushar's portfolio! I'm here to serve and help showcase my master's incredible skills!`,
  ...
  formatConversationForAPI() {
    const formattedMessages = this.conversationHistory.map((msg, index) => {
      ...
      if (msg.sender === 'user') {
        ...
        return `🤖 **AI Assistant** (${timestamp}):\n${msg.message}`;
      }
    }).join('\n\n---\n\n');
    
    const sessionInfo = `📋 **Chat Session Details**\n` +
                       `📅 Date: ${new Date().toLocaleDateString()}\n` +
                       `⏰ Session Time: ${new Date().toLocaleTimeString()}\n` +
                       `💬 Total Messages: ${this.conversationHistory.length}\n` +
                       `🌐 Source: Tushar's Portfolio Website\n\n`;
    
    const conversationHeader = `💬 **Conversation Log**\n\n`;
    
    return `${sessionInfo}${conversationHeader}${formattedMessages}\n\n` +
           `📝 **Session Summary**\n` +
           `• User Messages: ${this.conversationHistory.filter(msg => msg.sender === 'user').length}\n` +
           `• AI Responses: ${this.conversationHistory.filter(msg => msg.sender === 'bot').length}\n` +
           `• Conversation Duration: ${this.getConversationDuration()}\n\n` +
           `🔗 Generated by Tushar's AI Assistant`;
  }

  // Get conversation duration
  getConversationDuration() {
    if (this.conversationHistory.length < 2) return 'N/A';
    ...
    const duration = Math.round((lastMessage - firstMessage) / 1000); // in seconds
